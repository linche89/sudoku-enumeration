...
if (!(Test-Path "build/mp_fast.exe") -or !(Test-Path "build/mp_c6.exe") -or !(Test-Path "build/mp_q.exe") -or !(Test-Path "build/ms_fast.exe")) {
...
Invoke-Checked "mp_q C=2" { & ".\build\mp_q.exe" 2 20 nocheckpoint }
...
Invoke-Checked "mp_q histogram difftest C=4" { & ".\build\mp_q.exe" 4 difftest $HistTests }
Invoke-Checked "mp_q refinement canontest C=4" { & ".\build\mp_q.exe" 4 canontest $CanonTests }
