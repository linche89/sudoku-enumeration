...
Result run(int C,bool sw=true,int stop=-1){
    ...
    Canon can(C,sw);State z;std::unordered_map<Key,cpp_int,KeyHash>cur,nxt;cur.emplace(can.canonical(z),cpp_int(1));Result out;out.states.push_back(1);int lim=stop>=0?stop:2*C;
    for(int m=0;m<lim;m++){
        auto t0=std::chrono::steady_clock::now();nxt.clear();nxt.reserve(cur.size()*4+100);std::uint64_t valid=0;can.nodes=0;
        for(auto const&kv:cur){State s=unpack(kv.first,C);for(auto const&mv:ms){State t=s;int fac=1;for(int r=0;r<C;r++){int q=add_edge(t,C,r,mv.p[r],mv.q[r]);if(!q){fac=0;break;}fac*=q;}if(!fac)continue;valid++;Key k=can.canonical(t);nxt[k]+=kv.second*fac;}}
