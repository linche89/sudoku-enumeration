...
static inline void checked_add(std::uint64_t &dst, std::uint64_t a, int fac=1){
    __uint128_t v=(__uint128_t)a*(unsigned)fac + dst;
...
Result run(int C,bool sw=true,int stop=-1){
    ...
    Canon can(C,sw);State z;std::unordered_map<Key,std::uint64_t,KeyHash>cur,nxt;cur.emplace(can.canonical(z),std::uint64_t(1));Result out;out.states.push_back(1);int lim=stop>=0?stop:2*C;
    for(int m=0;m<lim;m++){
