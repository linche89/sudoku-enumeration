...
int run(int argc, char** argv) {
    ...
    if (C < 5 || C > 6 || !options.readonly || options.catalogue.empty() ||
        ...
        throw std::runtime_error("require bounded readonly C5/C6, <=1024 sources/5m records/24threads/55GiB/600s; C6 LOOKUP ONLY; C5 exact all355");
    ...
    std::cout << "source_sha256=" << catalogue.sourceSha256
...
int main(int argc, char** argv) try { return reverse_f5_bench::run(argc, argv); }
catch (const std::exception& exception) {
