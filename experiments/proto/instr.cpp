...
static unsigned long long g_canon=0, g_buildinner=0, g_merge=0;                       // M=2C symbols, FULLC=(1<<C)-1
...
static Key canonMS(const std::vector<int>& pairs, std::vector<int>* repOut=nullptr){
    ...
    for(size_t rp=0; rp<CP.size(); ++rp){
        ...
        for(size_t cp=0; cp<CP.size(); ++cp){ ++g_canon;
...
int main(int argc,char**argv){
    ...
    for(int band=0; band<C; ++band){
        ...
        for(auto&kv:states){
            ...
            for(int A:Askel){
                ...
                auto buildHist=[&](const int* syms,const std::vector<std::array<int,8>>& AX,const std::vector<std::array<int,8>>& AY){
                    ...
                    for(auto&ax:AX) for(auto&ay:AY){ ++g_buildinner;
                        PartKey part{};
                ...
                for(auto&tp:hTop) for(auto&bp:hBot){ ++g_merge;
        ...
        std::fprintf(stderr,"   [band %d] canonMSinner=%llu buildHistinner=%llu mergeCands=%llu\n",band,g_canon,g_buildinner,g_merge); g_canon=g_buildinner=g_merge=0; std::fflush(stderr);
    }
