...
static unsigned long long g_merge=0, g_canoncalls=0;
#include <set>
static std::set<std::array<uint16_t,12>> g_rawset;                 // all C-subsets of [2C]
...
static Key canonMS(const uint16_t* pairs, std::vector<int>* repOut=nullptr){
    ...
    for(size_t rp=0; rp<CP.size(); ++rp){
        ...
        for(size_t cp=0; cp<CP.size(); ++cp){
            ...
            for(int i=0;i<M;++i){ int t=pairs[i]; cand[i]=(uint16_t)mkPair(pmX[t&FULLC], pmY[(t>>C)&FULLC]); }
    ...
    for(int band=0; band<C; ++band){
        ...
        for(auto&kv:states){
            ...
            for(int A:Askel){
                ...
                uint16_t merged[MAXM];
                for(auto&tp:hTop) for(auto&bp:hBot){
                    for(int i=0;i<C;++i){ merged[i]=tp.first[i]; merged[C+i]=bp.first[i]; }
                    ++g_merge; { std::array<uint16_t,12> rk{}; uint16_t tmp[12]; for(int i=0;i<M;++i)tmp[i]=merged[i]; std::sort(tmp,tmp+M); for(int i=0;i<M;++i)rk[i]=tmp[i]; g_rawset.insert(rk); }
        ...
        std::fprintf(stderr,"band %d: states=%zu elapsed=%.1fs  mergeCands=%llu distinctRawMerged=%zu\n",band,states.size(),el,g_merge,g_rawset.size()); std::fflush(stderr); g_merge=0; g_rawset.clear();
