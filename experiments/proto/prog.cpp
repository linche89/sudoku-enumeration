...
int main(int argc,char**argv){
    ...
    for(int band=0; band<C; ++band){
        ...
        unsigned long long canonCalls=0;
        for(auto&kv:states){
            ++stateIdx;
            std::fprintf(stderr,"  band%d state %zu/%zu  canonMemo=%zu canonCalls=%llu  nx=%zu  el=%.1fs\n",
                band,stateIdx,nstates,canonMemo.size(),canonCalls,nx.size(),
