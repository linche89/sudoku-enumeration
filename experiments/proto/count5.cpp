...
static unsigned long long g_canonCalls=0;
#include <chrono>
static std::chrono::steady_clock::time_point g_t0;                 // all C-subsets of [2C]
...
static Key canonMS(const uint16_t* pairs, std::vector<int>* repOut=nullptr){
    if(((++g_canonCalls)&0xFFFFF)==0){ double el=std::chrono::duration<double>(std::chrono::steady_clock::now()-g_t0).count(); std::fprintf(stderr,"   canonCalls=%lluM el=%.1fs\n",g_canonCalls/1000000,el); std::fflush(stderr);}
...
int main(int argc,char**argv){
    ...
    initPerms(); g_t0=std::chrono::steady_clock::now();
    ...
    for(int band=0; band<C; ++band){
        ...
        for(auto&kv:states){
            ...
            for(int A:Askel){
                ...
                {
                    static unsigned long long g_merge=0, g_skel=0, g_maxprod=0;
                    g_merge += (unsigned long long)hTop.size()*hBot.size();
                    if((unsigned long long)hTop.size()*hBot.size() > g_maxprod){ g_maxprod=(unsigned long long)hTop.size()*hBot.size();
                        std::fprintf(stderr,"   NEW maxProd=%llu hTop=%zu hBot=%zu (band%d) el=%.1fs\n",g_maxprod,hTop.size(),hBot.size(),band,std::chrono::duration<double>(std::chrono::steady_clock::now()-g_t0).count()); std::fflush(stderr);}
                    if(((++g_skel)&0xFF)==0){ double el=std::chrono::duration<double>(std::chrono::steady_clock::now()-g_t0).count();
                        std::fprintf(stderr,"   skel=%llu mergeCandsCum=%lluM maxProd=%llu hTop=%zu hBot=%zu canonCalls=%llu el=%.1fs\n",
                            g_skel,g_merge/1000000,g_maxprod,hTop.size(),hBot.size(),g_canonCalls,el); std::fflush(stderr);}
                }
        ...
        std::fprintf(stderr,"band %d: states=%zu elapsed=%.1fs totalCanonCalls=%llu\n",band,states.size(),el,g_canonCalls); std::fflush(stderr);
